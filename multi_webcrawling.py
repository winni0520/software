...
while total_recipe_num >0:
    ...
    while category_url == "":
        if category == 1: #국/탕
            ...
            category == input("잘못된 입력입니다. 다시 입력해주세요. :")
    page = 1
    number = 0 #folder
    while category >0:
        ...
        titles = cate_soup.find_all("a", attrs={"class":"common_sp_link"})
        recipe_url = []
        for title in titles:
            url = title.get("href")
        ...
        if go_category >=1:
            for i in range(40):
                try:
                    ...
                    #shutil.rmtree(file_path+dir_name) #폴더 삭제
                    #number -=1 #folder number
                    print("-----------------------------------------")
                    print("error! "+folder_name+folder_number)
                    print("-----------------------------------------")
                    with open(file_path+dir_name+"\\error.txt", "w", encoding="utf-8") as f:
                        f.writelines("error")
                    recipe_num +=1
                    error +=1
                    continue
            ...
            go_category -=1
            total_recipe_num = total_recipe_num - 40
            ...
            for i in range(recipe_num):
                try:
                    ...
                    with open(file_path+dir_name+"\\error.txt", "w", encoding="utf-8") as f:
                        f.writelines("error")
                    recipe_num +=1 #에러난만큼 레시피 더 가져오기
                    continue
            if error != 0 :
                total_recipe_num = total_recipe_num- recipe_num +error
                error =0
            else: 
